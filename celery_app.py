...
def get_broker_url():
    ...
    # Force Redis for local development
    ...
    try:
        ...
        logger.warning(f"Redis not available ({e}), falling back to memory transport")
        return 'memory://'
...
def get_result_backend():
    ...
    try:
        ...
        logger.warning(f"Redis not available ({e}), falling back to memory backend")
        return 'cache+memory://'
