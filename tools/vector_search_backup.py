...
class VectorSearchTool:
    ...
    def __init__(self):
        # Temporarily disable vector search to allow system startup
        # Will be re-enabled once ML dependencies are installed
        self.pc = None
        self.index = None
        self.embedding_service = None
        logger.warning("Vector search temporarily disabled - placeholder mode")
    ...
    async def search(
        ...
        logger.warning(f"Vector search called with query: {query[:50]}... (placeholder mode)")
        return []
