...
async def process_slack_message(event_data: SlackEvent):
    ...
    try:
        ...
        if processed_message:
            ...
            if progress_updater:
                ...
                if response:
                    ...
                    # Update conversation activity (don't complete - let it continue)
                    # Conversations will timeout naturally after 30 minutes of inactivity
                else:
                    ...
                    # Log error but keep conversation active for potential retry
            else:
                ...
                if response:
                    ...
                    # Update conversation activity (fallback path)
                else:
                    # Log error but keep conversation active for potential retry (fallback path)
                    logger.warning("No response generated in fallback mode")
        else:
        ...
        # Log pipeline error but keep conversation active for potential retry
        logger.error(f"Pipeline error logged for conversation: {str(e)}")
