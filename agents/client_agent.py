...
class ClientAgent:
    ...
    async def generate_response(
        ...
        try:
            ...
            # If no response generated, try again with a more direct prompt
            logger.warning("Initial response generation failed, trying with free-form approach")
            
            # Create a simple, direct prompt that gives the AI full freedom
            direct_prompt = f"""
You are an intelligent AI assistant with expertise in Autopilot and automation. 

User's question: {message.text}
Channel: {message.channel_name}
User: {message.user_name}

Please provide a helpful, knowledgeable response using your expertise. Be conversational and helpful.
"""
            
            # Try again with the direct approach
            direct_response = await self.gemini_client.generate_response(
                "You are a helpful AI assistant with expertise in Autopilot and automation.",
                direct_prompt,
                model="gemini-2.5-flash",
                max_tokens=500
            )
            ...
            if direct_response:
                formatted_response = self._post_process_response(direct_response, message)
                suggestions = await self._generate_suggestions(message, gathered_info, context, formatted_response)
                return {
                    "text": formatted_response,
                    "suggestions": suggestions
            ...
            # Last resort: basic response acknowledging we received the message
            basic_response = f"I understand you're asking about: {message.text}. Let me help you with that using my knowledge of Autopilot and automation systems."
            suggestions = await self._generate_suggestions(message, gathered_info, context, basic_response)
            return {
                "text": basic_response,
                "suggestions": suggestions
            }
...
Provide a helpful, knowledgeable response to the user's question using all available information and your expertise.

Guidelines:
- Use the gathered information when available and relevant
- Draw on your knowledge of Autopilot, automation, and related technologies
- Be conversational, helpful, and engaging
- Provide practical insights and actionable guidance
- Format your response clearly for easy reading
- If specific information isn't available, provide general expertise and guidance on the topic
- Always aim to be helpful rather than highlighting limitations

Your goal is to be as helpful as possible using all available knowledge and information.
""")
