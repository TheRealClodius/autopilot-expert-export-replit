...
class TraceManager:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            try:
                ...
                # LangSmith create_run returns None by design (fire-and-forget), so test with a simple run
                ...
                # If no exception was raised, it's working
                self.enabled = True
                logger.info("LangSmith client fully functional (create_run successful)")
    ...
    async def start_conversation_session(
        ...
        try:
            ...
            # Create the trace with proper LangSmith format
            try:
                # Generate a UUID for this conversation trace
                trace_id = str(uuid.uuid4())
                
                # Create conversation trace in LangSmith with proper format
                self.client.create_run(
                    id=trace_id,  # Must provide ID
    ...
    async def log_user_message(self, user_id: str, message: str, timestamp: str) -> Optional[str]:
        ...
        try:
            ...
            # Create completed run in LangSmith with proper format
            self.client.create_run(
                id=message_id,  # Must provide ID
                name="user_message",
                inputs={
                    "user_id": user_id,
                    "message": message,
                ...
                project_name=self.settings.LANGSMITH_PROJECT,
                start_time=current_time,
                end_time=current_time,  # Complete immediately
